-- Asignacion automatica de sede (Medellín / Bogotá / Barranquilla) segun la ciudad
-- de la obra. Aplica tanto a la web como a up-whatsapp (ambos usan cotizar_web).
--   - Nueva columna cotizaciones_web.sede.
--   - Funcion sede_por_ciudad(ciudad): mapea ciudad/departamento -> sede (editable).
--   - cotizar_web calcula y guarda la sede, y la devuelve en el json de respuesta.
-- El PDF (print_cotizacion.html) y el mensaje de Liam por WhatsApp muestran la sede.

alter table cotizaciones_web add column if not exists sede text;

-- Mapea una ciudad/departamento a la sede que la atiende. Normaliza acentos/mayusculas.
-- Default: Barranquilla (sede base). Ajustar los patrones aqui si cambia la cobertura.
create or replace function public.sede_por_ciudad(p_ciudad text)
 returns text
 language plpgsql
 immutable
...
declare t text;
begin
  t := translate(lower(coalesce(p_ciudad, '')), 'áéíóúüñ', 'aeiouun');
  if length(trim(t)) = 0 then return 'Barranquilla'; end if;

  -- Costa Caribe -> Barranquilla
  if t ~ '(barranquilla|soledad|malambo|galapa|puerto colombia|sabanalarga|baranoa|palmar|santo tomas|sabanagrande|atlantic|santa marta|magdalena|cienaga|cartagena|bolivar|turbaco|valledupar|cesar|riohacha|guajira|maicao|sincelejo|sucre|monteria|cordoba|san andres|caribe|costa)' then
    return 'Barranquilla';
  end if;

  -- Antioquia / Eje Cafetero / Choco -> Medellín
  if t ~ '(medellin|envigado|itagui|\mbello|sabaneta|\mcaldas|rionegro|apartado|antioquia|uraba|copacabana|la estrella|girardota|pereira|risaralda|dosquebradas|manizales|armenia|quindio|quibdo|choco)' then
    return 'Medellín';
  end if;

  -- Centro / Oriente / Sur / Occidente -> Bogotá
  if t ~ '(bogota|soacha|\mchia|zipaquira|mosquera|funza|\mmadrid|facatativa|fusagasuga|girardot|cundinamarca|tunja|duitama|sogamoso|boyaca|villavicencio|\mmeta|acacias|bucaramanga|floridablanca|giron|piedecuesta|santander|cucuta|pamplona|ocana|ibague|tolima|espinal|neiva|huila|pasto|narino|popayan|cauca|cali|palmira|\mbuga|tulua|cartago|valle|yopal|casanare|arauca|florencia|caqueta|leticia)' then
    return 'Bogotá';
  end if;
...
-- cotizar_web: calcula y guarda la sede, y la devuelve en el json.
...
begin
  ...
  v_ciudad := coalesce(p_ciudad, v_cli.ciudad);
  v_sede := sede_por_ciudad(v_ciudad);
  ...
  for v_try in 1..8 loop
    ...
    begin
      insert into cotizaciones_web(
        id, nro, cliente, contacto, telefono, correo, ciudad, sede, nit, cliente_web_id,
        ...
        v_ciudad, v_sede, v_cli.nit, p_cliente_web_id,
  ...
  return json_build_object('ok', true, 'id', v_id, 'nro', v_nro, 'sede', v_sede,
